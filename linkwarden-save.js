...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (collectionsRes?.response) {
            const defaultCol = lastDefaults.linkwardenLastCollection || globals.linkwardenDefaultCollection || '';
            for (const c of collectionsRes.response) {
                const opt = document.createElement('option');
                opt.value = c.id;
                opt.textContent = c.name;
                if (c.id.toString() === defaultCol) opt.selected = true;
        ...
        const defaultTagNames = lastDefaults.linkwardenLastTags.length > 0
            ? lastDefaults.linkwardenLastTags
            : (globals.linkwardenDefaultTags || '').split(',').map(t => t.trim()).filter(Boolean);
