...
- **README and website hero image**: Replaced broken reference to removed `marquee_promo_tile_linkwarden.png` with existing `store-assets/promo-marquee.png` (README header, website hero, OG/Twitter meta, `prepare-website.sh`).
...
### Fixed
- **Chrome Web Store listing**: Rewrote store descriptions and manifest `extDescription` / `extDescriptionFirefox` in all 12 locales to remove excessive provider keyword repetition (Google rejection: Spam and Placement / Yellow Argon). Multi-provider support described generically; no functional change.
