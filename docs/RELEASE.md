...
|---|---|---|
| `3.0.0` | *GLaDOS* | Multi-provider Git sync: GitHub, GitLab, Codeberg, Gitea, Forgejo, Gogs (capability map, GitLab adapter with subgroups, shared provider UI). **Added:** profile bookmark transfer, push mirror destinations, clean remote orphans, live sync progress (`x / y` files/bookmarks). **Changed:** fast profile switch, transfer/sync progress ports, provider-neutral copy, full 12-language i18n (`v3.0.0-beta.2`). **Fixed:** GitLab empty-repo connection test, Gitea Contents-API writes/fallback, stale commit SHA, deleted bookmark reappearing after profile switch; setup wizard no longer auto-pushes on connection test ([#146](https://github.com/d0dg3r/GitSyncMarks/issues/146), `v3.0.0-beta.2`); Codeberg/Gitea repo-scoped PAT connection test (`v3.0.0-beta.3`); settings export/import and Git settings sync token API (`v3.0.0-beta.4`). Pre-releases: `v3.0.0-beta.1`, `v3.0.0-beta.2`, `v3.0.0-beta.3`, `v3.0.0-beta.4`. |
