...
|---|---|---|
| `3.0.0` | *GLaDOS* | Multi-provider Git sync: GitHub, GitLab, Codeberg, Gitea, Forgejo, Gogs (capability map, GitLab adapter with subgroups, shared provider UI). **Added:** profile bookmark transfer, push mirror destinations, clean remote orphans, live sync progress (`x / y` files/bookmarks); Gitea performance benchmark tooling (`v3.0.0-beta.5`); Bitwarden / Vaultwarden encrypted export backup to Git — upload, list, download, delete (`v3.0.0-beta.6`). **Changed:** fast profile switch, transfer/sync progress ports, provider-neutral copy, full 12-language i18n (`v3.0.0-beta.2`); Gitea-family git tree+blob reads and single-commit blob+tree writes (`v3.0.0-beta.5`); profile-switch step progress (`v3.0.0-beta.5`); unified nested-card UI across Options, wizard, popup, search (`v3.0.0-beta.7`); per-profile Bitwarden Git backup password (`v3.0.0-beta.7`); Bitwarden backup i18n in all 12 locales (`v3.0.0-beta.7`); What's new popup for 3.0 (`v3.0.0-beta.7`). **Fixed:** GitLab empty-repo connection test, Gitea Contents-API writes/fallback, stale commit SHA, deleted bookmark reappearing after profile switch; setup wizard no longer auto-pushes on connection test ([#146](https://github.com/d0dg3r/GitSyncMarks/issues/146), `v3.0.0-beta.2`); Codeberg/Gitea repo-scoped PAT connection test (`v3.0.0-beta.3`); settings export/import and Git settings sync token API (`v3.0.0-beta.4`); Codeberg CORS / network error on sync (`v3.0.0-beta.5`); Bitwarden Git-encryption download false “wrong password” (`v3.0.0-beta.6`); custom `bitwardenBackupPath` excluded from bookmark sync/orphan cleanup (`v3.0.0-beta.7`). Pre-releases: `v3.0.0-beta.1` … `v3.0.0-beta.7`. |
