...
Optional: [Chrome DevTools MCP](https://github.com/ChromeDevTools/chrome-devtools-mcp) and [Firefox DevTools MCP](https://github.com/mozilla/firefox-devtools-mcp) in **Cursor**. Copy the **reference** `mcpServers` object from [docs/TESTING.md](docs/TESTING.md#chrome-devtools-mcp-optional-cursor) into your user file `~/.cursor/mcp.json` (macOS / Linux) or `%USERPROFILE%\.cursor\mcp.json` (Windows) for a **user-wide** setup, or paste the same into a project’s `.cursor/mcp.json` (not both with identical names, or tools register twice; this repo’s `.cursor/mcp.json` starts empty to pair with a global file). They do **not** replace Playwright E2E (`npm run test:e2e*`, Chrome only).
