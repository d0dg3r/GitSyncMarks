...
Git リポジトリへのブックマーク同期。Linkwarden、Smart Search、Bitwarden バックアップ、ガイド付きウィザード。双方向、安全、プライベート。Firefox 完全対応。中間サーバーなし。

### Detailed Description
GitSyncMarks はブックマークを独自の Git リポジトリと双方向同期します — 主要ホスティングプラットフォームと self-hosted Git サーバー。中間サーバーなし、第三者サーバーなし – データは完全にあなたの管理下にあります。

ハイライト

- マルチプロバイダ Git 同期：Git ホストまたは self-hosted サーバーに接続 — プロファイルごとにプロバイダと URL を設定。プロバイダ一覧：https://github.com/d0dg3r/GitSyncMarks/blob/main/docs/PROVIDERS.md
...
- Bitwarden 互換 Git バックアップ：パスワード保護された vault エクスポートをリポジトリに保存、オプションの追加暗号化；リモートバックアップの一覧、ダウンロード、削除。
...
- 自動化：ブックマークの JSON を git または付属の GitHub Action テンプレートでリポジトリに追加すると、拡張機能が次回同期時にインポートします。
...
コンパニオンアプリ
GitSyncMarks-App（モバイルおよびデスクトップ）で Git リポジトリのブックマークを直接管理。（注：Firefox for Android は拡張機能による直接ブックマーク同期をサポートしていません – アプリをご利用ください。）
