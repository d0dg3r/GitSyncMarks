...
Git 저장소로 북마크 동기화. Linkwarden, Smart Search, Companion App. 직접, 안전, 비공개.

### Detailed Description
GitSyncMarks는 북마크를 자체 Git 저장소와 양방향 동기화합니다 — 주요 호스팅 플랫폼 및 self-hosted Git 서버. 데스크톱 브라우저 또는 GitSyncMarks Companion App으로 모바일에서 관리. 중개자 없음, 제3자 서버 없음 – 완전한 제어와 프라이버시.

하이라이트

- 멀티 프로바이더 Git 동기화: Git 호스트 또는 self-hosted 서버 연결 — 프로필마다 고유 프로바이더 및 URL 사용. 전체 프로바이더 목록: https://github.com/d0dg3r/GitSyncMarks/blob/main/docs/PROVIDERS.md
- 프로필 전송 및 푸시 미러: 프로필 간 북마크 복사(교체 또는 병합); 각 동기화 후 선택적 push-only 백업 원격.
- 실시간 동기화 진행: 푸시, 풀 및 프로필 전환 중 단계 텍스트(예: `3 / 12 파일` 또는 `1 / 3` 단계).
- Bitwarden 호환 Git 백업: 비밀번호로 보호된 vault 내보내기를 저장소에 저장, 선택적 추가 암호화; 원격 백업 나열, 다운로드 또는 삭제.
...
- Self-hosted Git 성능: 호환 호스트에서 빠른 git tree + blob 읽기 및 단일 커밋 푸시(필요 시 Contents API 폴백).
...
- 자동화: git 또는 포함된 GitHub Action 템플릿으로 북마크 JSON을 저장소에 추가하면, 확장 프로그램이 다음 동기화 시 가져옵니다.
- 생성 파일: README.md(개요), bookmarks.html(브라우저 가져오기), RSS 피드 및 dashy-conf.yml — 파일별 선택.
- 디자인 및 i18n: 라이트, 다크 및 시스템 자동 테마; 조절 가능한 UI 밀도(컴팩트 / 중간 / 큰); 12개 언어.

동반 앱
GitSyncMarks-App(모바일 및 데스크톱)으로 Git 저장소의 북마크를 직접 관리.
