...
GitSyncMarks는 북마크를 자체 Git 저장소와 양방향 동기화합니다 — 주요 호스팅 플랫폼 및 self-hosted Git 서버. Chrome과 Firefox 데스크톱 또는 GitSyncMarks Companion App으로 모바일에서 관리. 중개자 없음, 제3자 서버 없음 – 완전한 제어와 프라이버시.

하이라이트

- 멀티 프로바이더 Git 동기화: GitHub, GitLab 또는 self-hosted Git 연결 — 프로필마다 고유 프로바이더 및 서버 URL 사용 가능.
...
동반 앱
GitSyncMarks-App(Android, iOS, Desktop)으로 모바일에서 Git 저장소의 북마크를 직접 관리.
