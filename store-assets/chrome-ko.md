...
### 요약 (최대 132자)
GitHub 기반 북마크 동기화. Linkwarden 협업, 스마트 검색 및 컴패니언 앱. 직접적이고 안전하며 개인적입니다.

### 상세 설명
GitSyncMarks는 북마크를 GitHub 저장소와 자동 및 양방향으로 동기화하는 전문가용 솔루션입니다. 데스크톱(Chrome/Firefox)에서 데이터를 관리하거나, GitSyncMarks 컴패니언 앱을 통해 모바일에서도 편리하게 북마크에 액세스하세요. 중개 서버나 제3자 서버가 없으므로 완벽한 제어권과 프라이버시를 보장합니다.
...
- 멀티 프로필: 업무용, 개인용 등 최대 10개의 독립적인 프로필과 저장소를 관리할 수 있습니다.
- 자동화: CLI 또는 GitHub Actions를 통해 북마크를 추가하면 다음 동기화 시 자동으로 통합됩니다.
- 자동 생성 파일: 저장소에 README.md(개요), bookmarks.html(내보내기용), RSS 피드를 자동으로 생성합니다.
- 전문가급 디자인: 12개 언어를 지원하며 세련된 라이트/다크 테마를 제공합니다.

설치 및 설정

1. 설치: GitHub에서 ZIP 다운로드(Chrome/Edge) 또는 웹 스토어에서 설치하세요.
2. GitHub PAT: 'repo' 권한이 있는 개인 액세스 토큰을 생성하세요.
3. 설정 마법사: 도움말 -> 시작하기의 안내를 따르세요.
4. Linkwarden (선택): 전용 Linkwarden 탭에서 인스턴스를 설정하세요.

컴패니언 앱
GitSyncMarks-App(Android, iOS, Desktop)을 사용하여 모바일에서도 GitHub 저장소의 북마크를 직접 관리할 수 있습니다.
