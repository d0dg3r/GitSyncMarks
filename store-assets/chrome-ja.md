...
GitSyncMarks はブックマークを独自の Git リポジトリと双方向同期します — 主要ホスティングプラットフォームと self-hosted Git サーバー。デスクトップのブラウザ、または GitSyncMarks Companion App でモバイルから管理。中間サーバーなし、第三者サーバーなし – 完全なコントロールとプライバシー。
...
コンパニオンアプリ
GitSyncMarks-App（モバイルおよびデスクトップ）で Git リポジトリのブックマークを直接管理。
