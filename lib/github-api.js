...
export class GitHubAPI {
  ...
  async createRepository(params) {
    ...
    if (!response.ok) {
      ...
      const detail = err.message || 'Failed to create repository';
  ...
  async atomicCommit(message, fileChanges) {
    ...
    if (isEmptyRepo) {
      ...
      let baseTreeSha = currentTreeSha;
      let parentSha = currentCommitSha;
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          const isNonFastForward =
            ...
            err.statusCode === 422 &&
            (/fast.?forward|non-fast-forward|ref.*update/i.test(err.message) || err.message?.includes('fast forward'));
