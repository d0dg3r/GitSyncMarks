...
const MENU_IDS = {
  ...
  LINKWARDEN_SAVE_SCREENSHOT: 'gitsyncmarks-linkwarden-save-screenshot',
...
export function setupContextMenus() {
  ...
  browserObj.contextMenus.removeAll(async () => {
    ...
    const { linkwardenEnabled } = await browserObj.storage.sync.get({ linkwardenEnabled: false });
    ...
    for (const item of contextMenuItems) {
      ...
      switch (item.id) {
        ...
        case 'LINKWARDEN_SAVE':
          ...
          break;
        case 'LINKWARDEN_SAVE_SCREENSHOT':
          browserObj.contextMenus.create({
            id: MENU_IDS.LINKWARDEN_SAVE_SCREENSHOT,
            parentId: parentId,
            title: getMessage('contextMenu_saveToLinkwardenScreenshot'),
            contexts: ['page', 'link'],
            visible: linkwardenEnabled,
          });
          break;
  ...
  try {
    ...
    switch (info.menuItemId) {
      ...
      case MENU_IDS.LINKWARDEN_SAVE:
        await saveToLinkwarden(info, tab, { withScreenshot: false });
        break;
      case MENU_IDS.LINKWARDEN_SAVE_SCREENSHOT:
        await saveToLinkwarden(info, tab, { withScreenshot: true });
        break;
...
async function saveToLinkwarden(info, tab, options = {}) {
  try {
    ...
    if (!hasPermission) {
      const granted = await browserObj.permissions.request({ origins: [origin] });
      if (!granted) {
        await notify('Linkwarden permission denied', false);
        return;
      }
    }
