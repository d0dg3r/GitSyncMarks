...
export async function refreshContextMenuDynamicItems() {
  const runId = `pre-fix-refresh-${++refreshDebugSequence}`;
  // #region agent log
  postAgentDebugLog({
    runId,
    hypothesisId: 'H2',
    location: 'lib/context-menu.js:refreshContextMenuDynamicItems',
    message: 'refresh requested',
    data: {
      isRefreshing,
      refreshPending,
      currentProfileCount: currentProfileMenuIds.length,
      currentQuickCount: currentQuickFolderMenuIds.length,
      currentDynamicCount: currentDynamicFolderMenuIds.length,
      currentOpenAllCount: currentOpenAllFolderMenuIds.length,
    },
  });
  // #endregion
  if (isRefreshing) {
    ...
    postAgentDebugLog({
      ...
      hypothesisId: 'H2',
      location: 'lib/context-menu.js:refreshContextMenuDynamicItems',
      message: 'refresh queued because lock is active',
      data: { refreshPending },
    });
    // #endregion
    return;
  ...
  try {
    await Promise.all([
      refreshDynamicFolderMenu(runId),
      refreshQuickFolderMenuItems(runId),
      refreshOpenAllFolderMenuItems(runId),
      refreshProfileMenuItems(runId),
    ]);
  } catch (err) {
...
function debounceWithMaxWait(func, wait, maxWait) {
  ...
  return function debounced(...args) {
    ...
    if (typeof maxWait === 'number' && isFinite(maxWait)) {
      ...
      if (remaining <= 0) {
        // We've exceeded maxWait since the last actual invoke; flush now.
        invoke();
 ...
 */
export async function refreshProfileMenuItems(runId = 'pre-fix-unknown') {
  // #region agent log
  postAgentDebugLog({
    runId,
    hypothesisId: 'H5',
    location: 'lib/context-menu.js:refreshProfileMenuItems',
    message: 'profile refresh start',
    data: { trackedIdsCount: currentProfileMenuIds.length },
  });
  // #endregion
      ...
      browserObj.contextMenus.create({
        ...
        if (runtimeError) {
          ...
          postAgentDebugLog({
            runId,
            hypothesisId: 'H5',
            location: 'lib/context-menu.js:refreshProfileMenuItems:create',
            message: 'profile menu create error',
            data: { menuId, runtimeError, activeId },
          });
          // #endregion
        }
      });
...
async function refreshQuickFolderMenuItems(runId = 'pre-fix-unknown') {
...
async function refreshDynamicFolderMenu(runId = 'pre-fix-unknown') {
  // #region agent log
  postAgentDebugLog({
    runId,
    hypothesisId: 'H1',
    location: 'lib/context-menu.js:refreshDynamicFolderMenu',
    message: 'dynamic folder refresh start',
    data: { trackedIdsCount: currentDynamicFolderMenuIds.length },
  });
  // #endregion
        ...
        browserObj.contextMenus.create({
          ...
          if (runtimeError) {
            ...
            postAgentDebugLog({
              runId,
              hypothesisId: 'H4',
              location: 'lib/context-menu.js:refreshDynamicFolderMenu:create-folder',
              message: 'dynamic folder create error',
              data: { menuId, nodeId: node.id, runtimeError },
            });
            // #endregion
          }
        });
          ...
          browserObj.contextMenus.create({
            ...
            if (runtimeError) {
              ...
              postAgentDebugLog({
                runId,
                hypothesisId: 'H4',
                location: 'lib/context-menu.js:refreshDynamicFolderMenu:create-addhere',
                message: 'dynamic addhere create error',
                data: { addHereId, nodeId: node.id, runtimeError },
              });
              // #endregion
            }
          });
          ...
          browserObj.contextMenus.create({
            ...
            if (runtimeError) {
              ...
              postAgentDebugLog({
                runId,
                hypothesisId: 'H4',
                location: 'lib/context-menu.js:refreshDynamicFolderMenu:create-separator',
                message: 'dynamic separator create error',
                data: { sepId, nodeId: node.id, runtimeError },
              });
              // #endregion
            }
          });
...
async function refreshOpenAllFolderMenuItems(runId = 'pre-fix-unknown') {
  // #region agent log
  postAgentDebugLog({
    runId,
    hypothesisId: 'H1',
    location: 'lib/context-menu.js:refreshOpenAllFolderMenuItems',
    message: 'open-all refresh start',
    data: { trackedIdsCount: currentOpenAllFolderMenuIds.length },
  });
  // #endregion
      ...
      chrome.contextMenus.create({
        ...
        if (runtimeError) {
          ...
          postAgentDebugLog({
            runId,
            hypothesisId: 'H3',
            location: 'lib/context-menu.js:refreshOpenAllFolderMenuItems:create',
            message: 'open-all create error',
            data: { menuId, folderId: folder.id, runtimeError },
          });
          // #endregion
        }
      });
