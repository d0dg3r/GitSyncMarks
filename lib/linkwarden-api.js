...
export class LinkwardenAPI {
    ...
    async saveLink({ url, name, description, collectionId, tags }) {
        // Accept tags as plain strings or { name } objects (the auto-save mirror passes objects).
        const normalizeTag = (t) => (typeof t === 'string' ? t : (t && t.name) || '').trim();
        const payload = {
            url,
            name,
            description: description || '',
            tags: Array.isArray(tags) ? tags.map(t => ({ name: normalizeTag(t) })).filter(t => t.name) : []
        };
