...
const WHATS_NEW_BY_VERSION = {
  '3.0.7': {
    bullets: [
      'Settings import: after importing a backup from Files → Export/Import, the reload confirmation is visible again and the extension reloads so restored profiles apply.',
      'Settings import: full backup restore (profiles, mirrors, Linkwarden, onboarding flags) instead of a partial whitelist.',
      'Settings export: refuses to save a backup when configured profiles have no token, and includes mirror tokens in the file.',
    ],
  },
  '3.0.5': {
    bullets: [
      'Onboarding wizard: first sync (pull, push, or sync) no longer spins forever — the confirmation prompt is now visible inside the wizard before sync starts.',
    ],
  },
  '3.0.4': {
    bullets: [
