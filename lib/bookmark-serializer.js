...
function processFolder(folder, dirPath, files, options = {}) {
  ...
  for (const child of children) {
    if (child.url) {
      // Bookmark — skip if an identical filename was already emitted in this folder
      ...
      // Subfolder — check if excluded
      if (excludeFolders.includes(child.title)) {
        continue;
      }

      // store original title in the order entry
...
function renderFolderAsMarkdown(files, dirPath, lines, headingLevel) {
  ...
  // Include orphan subfolders: folders present in file map but missing from _order.json
  const dirPrefix = dirPath + '/';
  for (const path of Object.keys(files)) {
    if (!path.startsWith(dirPrefix)) continue;
    const rel = path.substring(dirPrefix.length);
    const parts = rel.split('/');
    if (parts.length >= 2 && parts[1] === '_order.json') {
      const folderName = parts[0];
      if (processedFolders.has(folderName)) continue;
...
function renderFolderAsNetscapeHtml(files, dirPath, parts, defaultDate) {
  ...
  // Include orphan subfolders
  const dirPrefix = dirPath + '/';
  for (const path of Object.keys(files)) {
    if (!path.startsWith(dirPrefix)) continue;
    const rel = path.substring(dirPrefix.length);
    const partsRel = rel.split('/');
    if (partsRel.length >= 2 && partsRel[1] === '_order.json') {
      const folderName = partsRel[0];
      if (processedFolders.has(folderName)) continue;
      const folderPath = `${dirPath}/${folderName}`;
      const title = escapeHtml(folderName.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()));
      parts.push(`        <DT><H3 FOLDED ADD_DATE="${defaultDate}">${title}</H3>`);
      parts.push('        <DL><p>');
      renderFolderAsNetscapeHtml(files, folderPath, parts, defaultDate);
      parts.push('        </DL><p>');
...
function collectRssItems(files, dirPath, categoryPath, items) {
  ...
  for (const path of Object.keys(files)) {
    ...
    if (pathParts.length >= 2 && pathParts[1] === '_order.json') {
...
function collectDashySections(files, dirPath, sectionName, sections) {
  ...
  for (const path of Object.keys(files)) {
    ...
    if (pathParts.length >= 2 && pathParts[1] === '_order.json') {
      ...
      const folderPath = `${dirPath}/${folderName}`;
      const title = folderName.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
      collectDashySections(files, folderPath, `${sectionName} > ${title}`, sections);
      processedFolders.add(folderName);
    }
  }
}
