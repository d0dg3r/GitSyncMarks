...
function processFolder(folder, dirPath, files, options = {}) {
  ...
  for (const child of rawChildren) {
    if (child.url) {
      ...
      if (folderTitleMap.has(title)) {
        ...
        // #region agent log
        console.info(`[GitSyncMarks][debug-f0a9cb] SERIALIZE_MERGE title="${title}" mergedChildCount=${existing.children.length} dirPath="${dirPath}"`);
        // #endregion
      } else {
  ...
  for (const child of mergedChildren) {
    if (child.url) {
      ...
      // #region agent log
      if (folderName !== slugify(child.title)) {
        console.info(`[GitSyncMarks][debug-f0a9cb] SERIALIZE_DEDUP title="${child.title}" slug="${slugify(child.title)}" folderName="${folderName}" dirPath="${dirPath}"`);
      }
      // #endregion
...
function buildFolderChildren(files, dirPath) {
    ...
    if (typeof entry === 'string' && entry.endsWith('.json')) {
      ...
      if (files[`${folderPath}/_order.json`] !== undefined) {
        ...
        if (seenFolderTitles.has(folderTitle)) {
          ...
          console.info(`[GitSyncMarks][debug-f0a9cb] DUPLICATE_FOLDER title="${folderTitle}" dir="${entry.dir}" firstDir="${children[existingIdx]?.title}" dirPath="${dirPath}" childrenCount=${folderChildren.length}`);
      ...
      if (files[`${folderPath}/_order.json`] !== undefined) {
        ...
        if (seenFolderTitles.has(folderTitle)) {
          const existingIdx = seenFolderTitles.get(folderTitle);
          const folderChildren = buildFolderChildren(files, folderPath);
          console.info(`[GitSyncMarks][debug-f0a9cb] DUPLICATE_FOLDER(str) title="${folderTitle}" dirPath="${dirPath}" childrenCount=${folderChildren.length}`);
          children[existingIdx].children.push(...folderChildren);
          continue;
        }
        seenFolderTitles.set(folderTitle, children.length);
  ...
  for (const orphan of findOrphanSubfolders(files, dirPath, processedFolders)) {
    // #region agent log
    const orphanTitle = orphan.title;
    if (seenFolderTitles.has(orphanTitle)) {
      const existingIdx = seenFolderTitles.get(orphanTitle);
      const folderChildren = buildFolderChildren(files, orphan.folderPath);
      console.info(`[GitSyncMarks][debug-f0a9cb] DUPLICATE_FOLDER(orphan) title="${orphanTitle}" folderPath="${orphan.folderPath}" childrenCount=${folderChildren.length}`);
      children[existingIdx].children.push(...folderChildren);
      continue;
    }
    seenFolderTitles.set(orphanTitle, children.length);
    // #endregion
    children.push({
      type: 'folder',
      title: orphan.title,
