...
export async function saveSyncState(profileId, api, basePath, localFiles, commitSha) {
  ...
  const { tree: treeEntries, truncated } = await api.getTree(treeSha);
  if (truncated) {
    // Base SHA map would be incomplete; missing SHAs just force a blob re-fetch
    // on the next sync (no data loss), so we proceed but record the condition.
    await debugLog('saveSyncState: tree truncated — base SHA map incomplete, blobs will be re-fetched next sync');
  }
