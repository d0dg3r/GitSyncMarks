...
/**
 * Recursively sort object keys so two JSON documents that differ only by key
 * order (or by formatting) canonicalize to the same string. Array order is
 * preserved because order is significant for _order.json.
 * @param {*} value
 * @returns {*}
 */
...
/**
 * Canonical JSON string for a content blob, or null if it is not valid JSON.
...
/**
 * Compare two file contents semantically. Identical strings short-circuit;
 * otherwise valid JSON is compared by canonical form so cosmetic differences
 * (key order, whitespace) do not register as changes or conflicts.
 * @param {string|null} a
 * @param {string|null} b
 * @returns {boolean}
 */
...
export async function push(options = {}) {
  ...
  try {
    ...
    const newCommitSha = await commitBookmarkChanges(api, commitMsg, fileChanges);
  ...
  try {
    ...
    if (!baseFiles) {
      ...
      if (!hasRemote && hasLocal) {
        ...
        return await push({ fromSync: true, connectionOverride: options.connectionOverride });
      }
      if (hasRemote && !hasLocal) {
    ...
    if (localHasChanges && !remoteHasChanges) {
      ...
      const newCommitSha = await commitBookmarkChanges(api, msg, fileChanges);
    ...
    if (Object.keys(toPush).length > 0) {
      ...
      const newCommitSha = await commitBookmarkChanges(api, msg, toPush);
      const commitMs = performance.now() - tCommit;
 ...
 * @returns {Promise<{success: boolean, message: string}>}
 */
export async function bootstrapFirstSync(connectionOverride = null) {
  let result = null;
  for (let attempt = 0; attempt < 4; attempt += 1) {
    result = await sync({ connectionOverride });
