...
const SETTINGS_ENC_PATTERN = /\/settings(-[a-f0-9]+)?\.enc$/;
...
/**
 * List all device-specific settings files from the remote repo.
 * @returns {Promise<Array<{filename: string, deviceId: string}>>}
 */
export async function listRemoteDeviceConfigs() {
  ...
  const results = [];
  const re = /\/settings-([a-f0-9]+)\.enc$/;
  for (const path of Object.keys(remote.fileMap)) {
    const m = path.match(re);
    if (m) results.push({ filename: path.split('/').pop(), deviceId: m[1] });
  }
  const globalPath = `${basePath}/settings.enc`;
  if (remote.fileMap[globalPath]) {
    results.unshift({ filename: 'settings.enc', deviceId: 'global' });
  }
  return results;
}

/**
 * Import and apply a specific device config file from the repo.
 * @param {string} filename - e.g. "settings-a1b2c3d4.enc" or "settings.enc"
 * @returns {Promise<{success: boolean, message: string}>}
 */
export async function importDeviceConfig(filename) {
  const settings = await getSettings();
  ...
  const encContent = remote.fileMap[fullPath];
