...
export async function getSettings() {
  ...
  return {
    ...
    generateReadmeMd: result.generateReadmeMd !== false,
    generateBookmarksHtml: result.generateBookmarksHtml !== false,
  };
}
 ...
 */
const DIFF_IGNORE_SUFFIXES = ['/README.md', '/_index.json', '/bookmarks.html'];
    ...
    if (remote) {
      for (const path of Object.keys(remote.fileMap)) {
        if (path.startsWith(basePath + '/') && !(path in localFiles) &&
            !path.endsWith('/README.md') && !path.endsWith('/bookmarks.html')) {
          fileChanges[path] = null; // delete
        }
      }
    }

    // Generate overview files (if enabled) — add before empty check so toggle-on always pushes
    if (settings.generateReadmeMd !== false) {
      fileChanges[`${basePath}/README.md`] = fileMapToMarkdown(localFiles, basePath);
    }
    if (settings.generateBookmarksHtml !== false) {
...
export async function sync() {
  ...
  try {
    ...
    if (localHasChanges && !remoteHasChanges) {
      ...
      // Regenerate overview files (if enabled)
      if (settings.generateReadmeMd !== false) {
        fileChanges[`${basePath}/README.md`] = fileMapToMarkdown(localFiles, basePath);
      }
      if (settings.generateBookmarksHtml !== false) {
    ...
    if (Object.keys(toPush).length > 0) {
      ...
      for (const path of Object.keys(remoteFiles)) {
        if (!(path in mergedFiles) && !path.endsWith('/README.md') && !path.endsWith('/bookmarks.html')) {
          toPush[path] = null;
        }
      }
      if (settings.generateReadmeMd !== false) {
