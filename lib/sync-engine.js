 ...
 */
const DIFF_IGNORE_SUFFIXES = ['/README.md', '/_index.json', '/bookmarks.html', '/feed.xml'];
    ...
    if (remote) {
      for (const path of Object.keys(remote.fileMap)) {
        if (path.startsWith(basePath + '/') && !(path in localFiles) &&
            !path.endsWith('/README.md') && !path.endsWith('/bookmarks.html') && !path.endsWith('/feed.xml')) {
  ...
  try {
    ...
    if (Object.keys(toPush).length > 0) {
      ...
      for (const path of Object.keys(remoteFiles)) {
        if (!(path in mergedFiles) && !path.endsWith('/README.md') && !path.endsWith('/bookmarks.html') && !path.endsWith('/feed.xml')) {
          toPush[path] = null;
        }
      }
      if (settings.generateReadmeMd === 'auto') {
