...
export async function getSettings() {
  ...
  return {
    ...
    settingsSyncMode: result.settingsSyncMode || 'global',
    settingsSyncGlobalWriteEnabled: result.settingsSyncGlobalWriteEnabled === true,
  };
}
 ...
 */
const DIFF_IGNORE_SUFFIXES = ['/README.md', '/_index.json', '/bookmarks.html', '/feed.xml', '/dashy-conf.yml', '/settings.enc'];
const SETTINGS_ENC_PATTERN = /\/settings(?:-[^/]+)?\.enc$/;
const SETTINGS_INDEX_FILENAME = 'settings-index.json';
...
async function applyRemoteSettings(json) {
  ...
  await chrome.storage.sync.set({
    ...
    settingsSyncMode: settings.settingsSyncMode || 'global',
...
async function buildEncryptedSettings(settings) {
  ...
  const mode = settings.settingsSyncMode || 'global';
  if (mode === 'global' && settings.settingsSyncGlobalWriteEnabled !== true) {
    await debugLog('settings-sync: global write disabled on this client');
    return null;
  }
  let filename = 'settings.enc';
  if (mode === 'individual') {
    const deviceId = await getDeviceId();
    filename = `settings-${deviceId.substring(0, 8)}.enc`;
  }
...
async function applyEncryptedSettings(encryptedContent, settings) {
  ...
  if ((settings.settingsSyncMode || 'global') === 'individual') {
  ...
  try {
    ...
    console.warn('[GitSyncMarks] settings.enc decrypt failed:', err.message);
  }
}

function settingsIndexPath(basePath) {
  return `${basePath}/${SETTINGS_INDEX_FILENAME}`;
}

function settingsFilePath(basePath, filename) {
...
function parseSettingsIndex(content) {
...
function buildSettingsIndex(entries) {
  return JSON.stringify({ version: 1, entries }, null, 2);
}

function buildProfilesFromRemote(remoteMap, basePath) {
  const indexed = new Map();
  const index = parseSettingsIndex(remoteMap[settingsIndexPath(basePath)]);
  for (const entry of index.entries) {
    if (!entry?.filename || !/^settings(?:-[^/]+)?\.enc$/.test(entry.filename)) continue;
    indexed.set(entry.filename, {
      id: entry.id || entry.filename.replace(/^settings-/, '').replace(/\.enc$/, '') || 'legacy',
      name: entry.name || entry.filename,
      filename: entry.filename,
      createdAt: entry.createdAt || null,
      updatedAt: entry.updatedAt || null,
      scope: entry.scope || (entry.filename === 'settings.enc' ? 'global' : 'client'),
    });
  }
  ...
  for (const path of Object.keys(remoteMap || {})) {
    ...
    if (!/^settings(?:-[^/]+)?\.enc$/.test(filename)) continue;
    files.push(filename);
  }
  ...
  for (const filename of files) {
    if (indexed.has(filename)) {
      out.push(indexed.get(filename));
    } else {
      out.push({
        id: filename.replace(/^settings-/, '').replace(/\.enc$/, '') || 'legacy',
        name: filename === 'settings.enc' ? 'Global (legacy)' : filename,
        filename,
        createdAt: null,
        updatedAt: null,
        scope: filename === 'settings.enc' ? 'global' : 'client',
      });
    }
  }

  out.sort((a, b) => {
    const ag = a.filename === 'settings.enc' ? 0 : 1;
    const bg = b.filename === 'settings.enc' ? 0 : 1;
...
export async function listSettingsProfilesFromRepo() {
  ...
  if (!remote || !remote.fileMap) return [];
  return buildProfilesFromRemote(remote.fileMap, basePath);
}
...
export async function listRemoteDeviceConfigs() {
  ...
  return profiles.map((p) => ({
    filename: p.filename,
    deviceId: p.filename === 'settings.enc'
      ? 'global'
      : (p.id || p.filename.replace(/^settings-/, '').replace(/\.enc$/, '')),
  }));
}
...
export async function importSettingsProfile(filename, passwordOverride = '') {
  ...
  const basePath = settings[STORAGE_KEYS.FILE_PATH].replace(/\/+$/, '');
  const remote = await fetchRemoteFileMap(api, basePath, null);
  if (!remote || !remote.fileMap) return { success: false, message: 'No remote data' };
  if (!/^settings(?:-[^/]+)?\.enc$/.test(filename)) return { success: false, message: 'Invalid settings file' };

  const encContent = remote.fileMap[settingsFilePath(basePath, filename)];
  if (!encContent) return { success: false, message: 'File not found in repo' };
  ...
  try {
    ...
    await applyRemoteSettings(json);
    suppressSettingsSyncUntil = Date.now() + 30000;
    await debugLog(`settings-sync: imported config from ${filename}`);
...
export async function syncCurrentSettingsToProfile({ filename, password, name = '' } = {}) {
  if (!filename || !/^settings(?:-[^/]+)?\.enc$/.test(filename)) return { success: false, message: 'Invalid settings file' };
  ...
  const basePath = settings[STORAGE_KEYS.FILE_PATH].replace(/\/+$/, '');
  const remote = await fetchRemoteFileMap(api, basePath, null);
  const profiles = buildProfilesFromRemote(remote?.fileMap || {}, basePath);
  const now = new Date().toISOString();

  const payload = await buildSettingsPayload();
  const encrypted = await encryptWithPassword(payload, password);
  const fileChanges = { [settingsFilePath(basePath, filename)]: encrypted };

  if (filename !== 'settings.enc') {
    const existing = profiles.find((p) => p.filename === filename);
    const entries = profiles
      .filter((p) => p.filename !== 'settings.enc')
      .map((p) => ({
        id: p.id,
        name: p.filename === filename ? (name || p.name || p.filename) : (p.name || p.filename),
        filename: p.filename,
        createdAt: p.createdAt || now,
        updatedAt: p.filename === filename ? now : (p.updatedAt || p.createdAt || now),
        scope: p.scope || 'client',
      }));
    if (!existing) {
      entries.push({
        id: filename.replace(/^settings-/, '').replace(/\.enc$/, ''),
        name: name || filename,
        filename,
        createdAt: now,
        updatedAt: now,
    ...
    fileChanges[settingsIndexPath(basePath)] = buildSettingsIndex(entries);
  }
...
export async function createSettingsProfile({ name, password } = {}) {
  ...
  if (!cleanName) return { success: false, message: 'Name required' };
  if (!password) return { success: false, message: 'Password required' };
  ...
  const remote = await fetchRemoteFileMap(api, basePath, null);
  const profiles = buildProfilesFromRemote(remote?.fileMap || {}, basePath);
  const existing = new Set(profiles.map((p) => p.filename));
  const baseSlug = slugifySettingName(cleanName);
  let slug = baseSlug;
  let filename = `settings-${slug}.enc`;
  let i = 2;
  while (existing.has(filename) || filename === 'settings.enc') {
    slug = `${baseSlug}-${i++}`;
    filename = `settings-${slug}.enc`;
  }
  ...
  entries.push({ id: slug, name: cleanName, filename, createdAt: now, updatedAt: now, scope: 'client' });

  const fileChanges = {
    [settingsFilePath(basePath, filename)]: encrypted,
    [settingsIndexPath(basePath)]: buildSettingsIndex(entries),
  };
  const deviceId = await getDeviceId();
  const msg = `Create settings profile from ${deviceId.substring(0, 8)} — ${new Date().toISOString()}`;
  await api.atomicCommit(msg, fileChanges);
  return { success: true, message: 'Settings profile created', filename };
}

export async function readSettingsIndex() {
  const settings = await getSettings();
  if (!isConfigured(settings)) return { version: 1, entries: [] };
  const api = createApi(settings);
  const basePath = settings[STORAGE_KEYS.FILE_PATH].replace(/\/+$/, '');
  const remote = await fetchRemoteFileMap(api, basePath, null);
  return parseSettingsIndex(remote?.fileMap?.[settingsIndexPath(basePath)] || '');
}

export async function writeSettingsIndex(entries) {
  const settings = await getSettings();
  if (!isConfigured(settings)) return { success: false, message: 'Not configured' };
  const api = createApi(settings);
  const basePath = settings[STORAGE_KEYS.FILE_PATH].replace(/\/+$/, '');
  const fileChanges = {
    [settingsIndexPath(basePath)]: buildSettingsIndex(Array.isArray(entries) ? entries : []),
  };
  const deviceId = await getDeviceId();
  const msg = `Update settings index from ${deviceId.substring(0, 8)} — ${new Date().toISOString()}`;
  await api.atomicCommit(msg, fileChanges);
  return { success: true, message: 'Settings index updated' };
}
...
export async function pull(options = {}) {
  ...
  try {
    ...
    const remoteSettingsEnc = remote.fileMap[`${basePath}/settings.enc`];
...
export async function sync() {
  ...
  try {
    ...
    if (!localHasChanges && remoteHasChanges) {
      ...
      const remoteSettingsEnc8 = remoteFiles[`${basePath}/settings.enc`];
    ...
    const remoteSettingsEnc9 = remoteFiles[`${basePath}/settings.enc`];
