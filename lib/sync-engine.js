 ...
 */
async function getLocalFileMap(basePath, settings = {}) {
...
export async function push(options = {}) {
  ...
  try {
    ...
    await mirrorToLinkwarden(fileChanges, settings);
    ...
    const profileId = (typeof settings !== 'undefined' && settings?.profileId) || await getActiveProfileId();
...
/**
 * Check available chrome.storage.local quota and warn in the debug log if low.
 * Non-fatal: quota errors should not prevent sync from running.
 */
...
export async function sync() {
  ...
  try {
    ...
    if (Object.keys(toPush).length > 0) {
      ...
      // Linkwarden Auto-Save (Mirroring)
      await mirrorToLinkwarden(toPush, settings);
...
// ---- Linkwarden Mirroring ----

/**
 * Scan the pushed file map for new/modified bookmarks and mirror them to Linkwarden if enabled.
 */
async function mirrorToLinkwarden(toPushMap, settings) {
  if (settings[STORAGE_KEYS.LINKWARDEN_ENABLED] === false) return;
  if (settings[STORAGE_KEYS.LINKWARDEN_AUTO_SAVE] !== true) return;

  const url = settings[STORAGE_KEYS.LINKWARDEN_URL];
  // Re-fetch token directly from storage since `getSettings()` strips it if it isn't the GitHub token
  const globals = await chrome.storage.sync.get({
    linkwardenToken: '',
    linkwardenEnabled: false,
    linkwardenAutoSave: false
  });
  const encToken = globals.linkwardenToken;
  if (!url || !encToken) return;

  try {
    const plainToken = await decryptToken(encToken);
    const api = new LinkwardenAPI(url, plainToken);
    const defaultCollection = settings[STORAGE_KEYS.LINKWARDEN_DEFAULT_COLLECTION];
    const defaultTagsInput = settings[STORAGE_KEYS.LINKWARDEN_DEFAULT_TAGS] || '';
