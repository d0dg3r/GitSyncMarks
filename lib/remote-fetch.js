...
export async function fetchRemoteFileMap(api, basePath, baseFiles) {
  ...
  const { tree: treeEntries, truncated } = await api.getTree(commit.treeSha);
  // A truncated tree means we cannot see the full remote state. Proceeding would
  // misdiagnose missing files as deletions and could wipe remote data on cleanup.
  if (truncated) {
