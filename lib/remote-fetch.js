...
import { usesContentsApiReads } from './git-provider-common.js';
...
export async function fetchTreeEntriesForCommit(api, commitSha) {
  if (usesContentsApiReads(api.providerId)) {
    throw new GitHubError('Gitea-family providers use Contents API for remote reads', 422);
  }
...
export async function buildRemoteMaps(api, basePath, baseFiles, commitSha) {
  ...
  // Gitea-family: Contents API is reliable for reads; git/trees metadata is often missing.
  if (usesContentsApiReads(api.providerId) && typeof api.fetchFileMapViaContents === 'function') {
  ...
  try {
    ...
    if (Object.keys(shaMap).length === 0) {
      // Repo has commits but nothing under basePath yet (e.g. README-only GitLab repo).
      // Only try Contents-API fallback when that adapter exists (Gitea-family uses it elsewhere).
      if (typeof api.fetchFileMapViaContents === 'function') {
