...
import { GitProviderError, GIT_PROVIDERS, resolveApiBase, resolveWebBaseUrl } from '../git-provider-common.js';
...
export class GitHubAPI {
  /**
   * @param {string|import('../git-provider.js').GitProviderOptions} tokenOrOptions - PAT or options object
   * @param {string} [owner] - Repository owner (user or org)
   * @param {string} [repo] - Repository name
   * @param {string} [branch] - Branch name (default: 'main')
   */
  constructor(tokenOrOptions, owner, repo, branch = 'main') {
    ...
    this.providerId = (typeof tokenOrOptions === 'object' && tokenOrOptions?.provider) || GIT_PROVIDERS.GITHUB;
  }
  ...
  async _fetch(url, options = {}, _attempt = 0) {
    ...
    try {
      ...
      if (remaining !== null && parseInt(remaining, 10) <= 50) {
        ...
        console.warn(`[GitSyncMarks] [GitHub API] Only ${remaining} requests remaining. Resets at ${resetDate}`);
      }
    } catch (e) {
    ...
    if (isRateLimited) {
      ...
      if (_attempt < RATE_LIMIT_MAX_RETRIES && waitMs > 0 && waitMs <= RATE_LIMIT_MAX_WAIT_MS) {
        console.warn(`[GitSyncMarks] [GitHub API] Rate limited; retrying in ${waitMs}ms (attempt ${_attempt + 1}/${RATE_LIMIT_MAX_RETRIES})`);
        await sleep(waitMs);
  ...
  async listUserRepos({ perPage = 100 } = {}) {
    ...
    let hasMore = true;
    while (hasMore) {
      const url = `${this._apiBase()}/user/repos?per_page=${perPage}&type=all&page=${page}`;
  ...
  async checkRepo() {
    try {
      const response = await this._fetch(`${this._apiBase()}/repos/${this.owner}/${this.repo}`);
      return response.ok;
    } catch {
      return false;
    }
  ...
  async createRepository(params) {
    ...
    const response = await this._fetch(`${this._apiBase()}/user/repos`, {
      method: 'POST',
      body: JSON.stringify({
        name: repoName,
  ...
  async listContents(path = '') {
    ...
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/contents${urlPath}?ref=${this.branch}`;
    const response = await this._fetch(url);
   ...
   */
  async getFile(path) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/contents/${path}?ref=${this.branch}`;
    const response = await this._fetch(url);
  ...
  async createOrUpdateFile(path, content, message, sha = null) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/contents/${path}`;
    ...
    const data = await response.json();
    const commitSha = data.commit?.sha || data.commit_sha || '';
    const contentSha = data.content?.sha || data.sha || '';
    if (!commitSha) {
      throw new GitHubError(
  ...
  async getLatestCommitSha() {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/ref/heads/${this.branch}`;
    const response = await this._fetch(url);
  ...
  async getCommit(commitSha) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/commits/${commitSha}`;
    const response = await this._fetch(url);
    if (!response.ok) {
  ...
  async getCommitTreeSha(commitSha) {
    if (this._lastCommit && this._lastCommit.commitSha === commitSha && this._lastCommit.treeSha) {
  ...
  async getTree(treeSha) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/trees/${treeSha}?recursive=1`;
    const response = await this._fetch(url);
    if (!response.ok) {
  ...
  async getBlob(blobSha) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/blobs/${blobSha}`;
    const response = await this._fetch(url);
    if (!response.ok) {
  ...
  async createBlob(content) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/blobs`;
    const response = await this._fetch(url, {
      method: 'POST',
  ...
  async createTree(baseTreeSha, items) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/trees`;
  ...
  async createCommit(message, treeSha, parentSha = null) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/commits`;
    const body = {
      message,
      tree: treeSha,
  ...
  async updateRef(commitSha) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/refs/heads/${this.branch}`;
    const response = await this._fetch(url, {
      method: 'PATCH',
  ...
  async createRef(commitSha) {
    const url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/git/refs`;
    const response = await this._fetch(url, {
      method: 'POST',
      body: JSON.stringify({
  ...
  async listCommits({ path, perPage = 20 } = {}) {
    let url = `${this._apiBase()}/repos/${this.owner}/${this.repo}/commits?sha=${this.branch}&per_page=${perPage}`;
  ...
  async atomicCommit(message, fileChanges, onProgress) {
    ...
    if (isEmptyRepo) {
      ...
      const sha = await commitOnExistingBranch(currentTreeSha, currentCommitSha);
      onProgress?.(total);
      return sha;
    }
  }
 ...
 */
export class GitHubError extends GitProviderError {
