 ...
 * Uses GitHub-compatible git data APIs for reads and writes; Contents API is the write fallback.
 */

import { getMessage } from '../i18n.js';
import { chunkAtomicCommitShaTreeBatches } from '../github-tree-batch.js';
...
/** Max parallel POST /git/blobs during Gitea-family atomic commits. */
const GITEA_BLOB_CREATE_CONCURRENCY = 5;

/** HTTP statuses that trigger Contents-API fallback after git-data write failure. */
 ...
 * @returns {boolean}
 */
function shouldFallbackContentsWrite(err) {
  const isModifiedConflict = /modified in the meantime/i.test(String(/** @type {{ message?: string }} */ (err)?.message || ''));
  return (
 ...
 */
function warnGiteaContentsWriteFallback(err, fileCount) {
  const detail =
    err instanceof GitHubError
      ? `HTTP ${err.statusCode}: ${err.message}`
      : String(/** @type {{ message?: string }} */ (err)?.message || err);
  console.warn(
    `[GitSyncMarks] Gitea git-data write failed (${detail}); ` +
      `using Contents API fallback for ${fileCount} file(s) — one commit per file. ` +
      'Reload the extension after updates if you expected a single commit.'
  );
  import('../debug-log.js')
    .then(({ log }) =>
      log(`[gitea-write] git-data failed (${detail}); Contents API fallback, ${fileCount} file(s)`)
    )
    .catch(() => {});
}
...
export class GiteaAPI extends GitHubAPI {
  ...
  constructor(options) {
    super(options);
    this.providerId = options.provider || GIT_PROVIDERS.GITEA;
    if (!this.serverUrl) {
      const caps = getProviderCaps(this.providerId);
      this.serverUrl = caps.defaultServerUrl || '';
    }
  }
  ...
  /** @returns {string} */
  effectiveServerUrl() {
    return resolveEffectiveServerUrl(this.providerId, this.serverUrl);
  }
  ...
  /**
   * Gitea/Codeberg: do not throw on 401/403 here — callers inspect response.ok.
   * Scoped tokens (e.g. read:repository without read:user) return 403 on /user
   * but still work for repo endpoints (Codeberg, Gitea ≥1.22).
   * @param {string} url
  ...
  async validateToken() {
    try {
      ...
      if (!response.ok) {
        ...
        // read:repository-only tokens (common on Codeberg) cannot call GET /user.
        if (response.status === 403) {
          return { valid: true, username: null, scopes: [], ambiguous: true };
        }
        return { valid: false, username: null, scopes: [] };
      }
  ...
  /**
   * Commit file changes: try git data (batched blobs + layered trees, one commit), then Contents API fallback.
   *
   * @param {string} message
  ...
  async atomicCommit(message, fileChanges, onProgress) {
    ...
    onProgress?.(0);
    try {
      const sha = await this._atomicCommitViaGitData(message, fileChanges, onProgress, total);
      onProgress?.(total);
      return sha;
    } catch (err) {
      if (!shouldFallbackContentsWrite(err)) throw err;
      warnGiteaContentsWriteFallback(err, total);
      const { pathToSha } = await this._isEmptyRepo()
        ? { pathToSha: {} }
        : await this._getPathShaMap();
      return this._atomicCommitSequential(message, fileChanges, pathToSha, onProgress);
    }
  }

  /**
   * Single-commit write via POST /git/blobs + layered POST /git/trees (SHA refs, no inline content).
   * @param {string} message
   * @param {Object<string, string|null>} fileChanges
   * @param {(current: number) => void} [onProgress]
   * @param {number} total
   * @returns {Promise<string>}
   */
  async _atomicCommitViaGitData(message, fileChanges, onProgress, total) {
    const commitOnExistingBranch = async (baseTreeSha, parentSha, batches) => {
    ...
    const uploadsWithSha = [];
    let blobsDone = 0;
    for (let i = 0; i < uploads.length; i += GITEA_BLOB_CREATE_CONCURRENCY) {
      const chunk = uploads.slice(i, i + GITEA_BLOB_CREATE_CONCURRENCY);
      ...
      uploadsWithSha.push(...chunkResults);
      blobsDone += chunk.length;
      onProgress?.(Math.min(blobsDone, total));
    }

    const batches = chunkAtomicCommitShaTreeBatches(deletions, uploadsWithSha);
      ...
      for (let attempt = 0; attempt < 4; attempt++) {
        ...
        try {
          ...
          try {
            ...
            return await commitOnExistingBranch(freshCommit.treeSha, freshCommit.sha, batches);
          } catch (readErr) {
    ...
    return commitOnExistingBranch(currentTreeSha, currentCommitSha, batches);
  }
  ...
  /** @param {string} message @param {Object<string, string|null>} fileChanges @param {Record<string, string>} pathToSha @param {(current: number) => void} [onProgress] @returns {Promise<string>} */
  async _atomicCommitSequential(message, fileChanges, pathToSha, onProgress) {
