...
// Roles that don't exist in Chrome; map to fallback parent + display title
const ROLE_FALLBACK = {
  menu: { parent: 'other', title: 'Bookmarks Menu' },
};
...
export async function replaceLocalBookmarks(roleMap, options = {}) {
  ...
  for (const [role, localFolder] of Object.entries(localByRole)) {
    ...
    // Fallback: Rollen ohne lokales Äquivalent als Unterordner einbinden
    for (const [missingRole, fallback] of Object.entries(ROLE_FALLBACK)) {
      if (fallback.parent === role && roleMap[missingRole] && !localByRole[missingRole]) {
        const extra = roleMap[missingRole];
        data = {
          ...data,
          children: [
            ...(data.children || []),
            { type: 'folder', title: fallback.title, children: extra.children || [] },
          ],
        };
      }
    }
