 ...
 * @param {{githubReposEnabled?: boolean, githubReposParent?: string, githubReposUsername?: string, linkwardenSyncEnabled?: boolean, linkwardenSyncParent?: string, linkwardenSyncPushToGit?: boolean}} [options]
 */
export async function replaceLocalBookmarks(roleMap, options = {}) {
  ...
  for (const role of SYNC_ROLES) {
    ...
    // #region agent log
    const folderTitleCounts = {};
    if (data.children) {
      for (const child of data.children) {
        if (child.type === 'folder') {
          folderTitleCounts[child.title] = (folderTitleCounts[child.title] || 0) + 1;
        }
      }
      const dupTitles = Object.entries(folderTitleCounts).filter(([, c]) => c > 1);
      if (dupTitles.length > 0) {
        console.info(`[GitSyncMarks][debug-f0a9cb] REPLACE_DUP_FOLDERS role="${role}" dups=${JSON.stringify(dupTitles)}`);
      }
    }
    // #endregion
    if (data.children) {
