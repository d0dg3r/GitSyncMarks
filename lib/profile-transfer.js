 ...
 * @returns {Object<string, string>}
 */
export function filterGeneratedAndAuto(fileMap, bitwardenBackupPath) {
  const out = {};
  for (const [path, content] of Object.entries(fileMap)) {
    if (isGeneratedOrSettingsPath(path, bitwardenBackupPath)) continue;
...
export async function previewTransfer(params, onProgress) {
  ...
  const sourceProfile = profiles[sourceId];
  ...
  let payload = filterGeneratedAndAuto(source.fileMap, sourceProfile.bitwardenBackupPath);
  ...
  const targetMap = filterGeneratedAndAuto(targetLoad.fileMap, targetProfile.bitwardenBackupPath);
