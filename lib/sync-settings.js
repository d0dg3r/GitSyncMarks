...
import { encryptWithPassword, decryptWithPassword, encryptToken } from './crypto.js';
import { getProfileSettings, setEncryptedProfileToken } from './profile-manager.js';
import {
  buildExportedProfileMap,
  listConfiguredProfilesMissingToken,
  restoreProfileTokensFromExport,
} from './settings-export.js';
import {
  DIFF_IGNORE_SUFFIXES,
  SETTINGS_ENC_PATTERN,
  BITWARDEN_BACKUP_DEFAULT_PATTERN,
  isBitwardenBackupDiffPath,
  filterForDiff,
} from './sync-diff.js';
...
import { STORAGE_KEYS, LOCAL_STORAGE_KEYS } from './storage-keys.js';

export { STORAGE_KEYS, LOCAL_STORAGE_KEYS };
...
export async function getSettings() {
  return getSettingsForProfile(null);
}

/**
 * Build sync settings object for a specific profile (or active when profileId is null).
 * @param {string|null} profileId
 * @returns {Promise<Record<string, unknown>|null>}
 */
export async function getSettingsForProfile(profileId) {
  const result = await getProfileSettings(profileId);
  ...
  return {
    ...
    [STORAGE_KEYS.GIT_PROVIDER]: result.gitProvider || 'github',
    [STORAGE_KEYS.SERVER_URL]: result.serverUrl || '',
    ...
    bitwardenBackupPath: result.bitwardenBackupPath || 'backups/bitwarden',
  };
}

export function isConfigured(settings) {
  const provider = settings[STORAGE_KEYS.GIT_PROVIDER] || 'github';
  ...
  if (caps.requireServerUrl) {
    const serverUrl = settings[STORAGE_KEYS.SERVER_URL] || caps.defaultServerUrl || '';
...
export function createApi(settings) {
  return createGitProvider({
    provider: settings[STORAGE_KEYS.GIT_PROVIDER] || 'github',
    token: settings[STORAGE_KEYS.GITHUB_TOKEN],
    owner: settings[STORAGE_KEYS.REPO_OWNER],
    repo: settings[STORAGE_KEYS.REPO_NAME],
    branch: settings[STORAGE_KEYS.BRANCH] || 'main',
    serverUrl: settings[STORAGE_KEYS.SERVER_URL] || '',
  });
}

/**
 * Merge optional connection form fields over persisted settings (used when the
 * background sync starts immediately after options save).
 * @param {Record<string, unknown>} settings
 * @param {import('./connection-settings.js').ConnectionFormFields|null|undefined} override
 * @returns {Record<string, unknown>}
 */
export function applyConnectionOverride(settings, override) {
  if (!override?.token) return settings;
  return {
    ...settings,
    [STORAGE_KEYS.GITHUB_TOKEN]: override.token,
    [STORAGE_KEYS.REPO_OWNER]: override.owner || settings[STORAGE_KEYS.REPO_OWNER],
    [STORAGE_KEYS.REPO_NAME]: override.repo || settings[STORAGE_KEYS.REPO_NAME],
    [STORAGE_KEYS.BRANCH]: override.branch || settings[STORAGE_KEYS.BRANCH] || 'main',
    [STORAGE_KEYS.GIT_PROVIDER]: override.gitProvider || settings[STORAGE_KEYS.GIT_PROVIDER] || 'github',
    [STORAGE_KEYS.SERVER_URL]: override.serverUrl ?? settings[STORAGE_KEYS.SERVER_URL] ?? '',
  };
}
 ...
 */
export function isGeneratedOrSettingsPath(path, bitwardenBackupPath) {
  if (DIFF_IGNORE_SUFFIXES.some((suffix) => path.endsWith(suffix))) return true;
  if (SETTINGS_ENC_PATTERN.test(path)) return true;
  if (isBitwardenBackupDiffPath(path, bitwardenBackupPath)) return true;
  return false;
}

/**
 * Strip volatile, regeneration-only timestamps from generated files so two
 * outputs that only differ by "now" compare as equal. Used to avoid committing
 * generated files on every sync when their meaningful content is unchanged.
 * @param {string} path
 * @param {string} content
 * @returns {string}
 */
function normalizeGeneratedContent(path, content) {
  if (typeof content !== 'string') return content;
  if (path.endsWith('/README.md')) {
    return content.replace(/^> Last synced: .*$/m, '> Last synced:');
  }
  if (path.endsWith('/bookmarks.html')) {
    return content.replace(/ ADD_DATE="\d+"/g, '').replace(/ LAST_MODIFIED="\d+"/g, '');
  }
  ...
  if (path.endsWith('/dashy-conf.yml')) {
    return content.replace(/^\s*exportedAt:.*$/m, '');
  }
  return content;
}

/**
 * Whether a freshly generated file differs meaningfully from the existing one.
 * @param {string} path
 * @param {string} newContent
 * @param {string|undefined|null} existingContent
 * @returns {boolean}
 */
function generatedContentChanged(path, newContent, existingContent) {
  if (existingContent === undefined || existingContent === null) return true;
  return normalizeGeneratedContent(path, newContent) !== normalizeGeneratedContent(path, existingContent);
}

/**
 * Add generated overview files to a file-changes map based on settings mode.
 * When `existingFiles` is provided, a generated file is only added if it changed
 * (ignoring volatile timestamps), avoiding needless commits/blob churn on sync.
 ...
 * @param {Object<string, string>|null} [existingFiles] - Current remote/known files to diff against
 */
export function addGeneratedFiles(fileChanges, sourceFiles, basePath, settings, threshold = 'auto', existingFiles = null) {
  const check = threshold === 'auto' ? (v) => v === 'auto' : (v) => v !== 'off';
  const maybeAdd = (relPath, content) => {
    const path = `${basePath}/${relPath}`;
    if (existingFiles && !generatedContentChanged(path, content, existingFiles[path])) return;
    fileChanges[path] = content;
  };
  if (check(settings.generateReadmeMd)) {
    maybeAdd('README.md', fileMapToMarkdown(sourceFiles, basePath));
  }
  if (check(settings.generateBookmarksHtml)) {
    maybeAdd('bookmarks.html', fileMapToNetscapeHtml(sourceFiles, basePath));
  }
  if (check(settings.generateFeedXml)) {
    maybeAdd('feed.xml', fileMapToRssFeed(sourceFiles, basePath));
  }
  if (check(settings.generateDashyYml)) {
    maybeAdd('dashy-conf.yml', fileMapToDashyYaml(sourceFiles, basePath));
  }
}

function isBookmarkPayloadPath(path) {
...
export function hasBookmarkPayloadFiles(files = {}) {
  return Object.keys(files).some(isBookmarkPayloadPath);
}

export function countBookmarkPayloadFiles(files = {}) {
  return Object.keys(files).filter(isBookmarkPayloadPath).length;
}
 ...
 * @returns {Promise<string|null>} JSON string, or null when configured profiles lack tokens
 */
async function buildSettingsPayload() {
  ...
  const exportedProfiles = await buildExportedProfileMap(profiles);
  const missing = listConfiguredProfilesMissingToken(exportedProfiles);
  if (missing.length) {
    await debugLog(`settings-sync: skipped push (missing tokens for: ${missing.join(', ')})`);
    return null;
  }
...
async function applyRemoteSettings(json) {
  ...
  for (const [id, p] of Object.entries(settings.profiles)) {
    const {
      token: _profileToken,
      githubToken: _profileGithubToken,
      mirrorTokens: _mirrorTokens,
    ...
    normalizedProfiles[id] = {
      ...
      mirrors: Array.isArray(p.mirrors) ? p.mirrors : [],
      contextQuickFolderIds: Array.isArray(p.contextQuickFolderIds)
        ? p.contextQuickFolderIds.slice(0, 3)
        : [],
    };
    await restoreProfileTokensFromExport(id, p, encryptToken, setEncryptedProfileToken);
  }

  const {
    profiles: _ignoredProfiles,
    token: _topToken,
    githubToken: _topGithubToken,
    ...rest
  } = settings;
  const hasConfiguredProfile = Object.values(normalizedProfiles).some((p) => p.owner && p.repo);
  await chrome.storage.sync.set({
    ...
    settingsSyncMode: 'individual',
    settingsSyncGlobalWriteEnabled: false,
    onboardingWizardCompleted: settings.onboardingWizardCompleted === true || hasConfiguredProfile,
    onboardingWizardDismissed: settings.onboardingWizardDismissed === true,
  });
...
export async function syncCurrentSettingsToProfile({ filename, password } = {}) {
  ...
  const payload = await buildSettingsPayload();
  if (!payload) {
    return { success: false, message: 'Cannot sync settings: one or more configured profiles are missing a token' };
  }
...
export async function createSettingsProfile({ name, password } = {}) {
  ...
  const payload = await buildSettingsPayload();
  if (!payload) {
    return { success: false, message: 'Cannot create settings profile: one or more configured profiles are missing a token' };
  }
  const encrypted = await encryptWithPassword(payload, password);
