...
import { getProfileSettings, getProfileToken, setEncryptedProfileToken } from './profile-manager.js';
import {
...
export function isGeneratedOrSettingsPath(path, bitwardenBackupPath) {
  ...
  if (bitwardenBackupPath) {
    const prefix = `${String(bitwardenBackupPath).replace(/^\/+|\/+$/g, '')}/`;
    if (prefix !== 'backups/bitwarden/' && path.startsWith(prefix)) return true;
  }
  return false;
}
...
async function buildSettingsPayload() {
  ...
  for (const [id, p] of Object.entries(profiles)) {
    const plainToken = await getProfileToken(id, 'primary');
