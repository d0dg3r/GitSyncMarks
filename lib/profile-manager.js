...
export async function switchProfile(targetId, options = {}) {
  ...
  // When switching to an empty, unconfigured profile: skip GitHub push to avoid slow API calls.
  // Current profile's data is already saved to local lastSyncFiles; push happens on next sync or switch.
  ...
  if (
    ...
    try {
      ...
      const { shaMap } = await fetchRemoteFileMapAtCommit(api, basePath, commitSha, { skipCache: true });
      const syncFiles = {};
      for (const [path, content] of Object.entries(localFiles)) {
