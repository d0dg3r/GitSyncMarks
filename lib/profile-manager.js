...
export async function switchProfile(targetId, options = {}) {
  ...
  if (
    ...
    try {
      ...
      const { fileChanges, hasChanges } = buildSwitchPushChanges(localFiles, currentState.lastSyncFiles);
      ...
      if (!hasChanges) {
        ...
        const commitSha = await commitBookmarkChanges(api, msg, fileChanges);
