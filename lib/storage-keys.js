...
/**
 * Per-profile local storage key for the optional Git backup wrap password.
 * @param {string} profileId
 * @returns {string}
 */
export function bitwardenBackupPasswordKey(profileId) {
  const id = profileId && String(profileId).trim() ? String(profileId).trim() : 'default';
  return `${LOCAL_STORAGE_KEYS.BITWARDEN_BACKUP_PASSWORD}:${id}`;
}
