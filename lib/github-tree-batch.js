...
/** @typedef {{ path: string, sha: string }} ShaUploadEntry */

/**
 * Split atomic-commit changes into batches for POST /git/trees when blobs were created via POST /git/blobs.
 * Used by Gitea-family {@link import('./providers/gitea-api.js').GiteaAPI#atomicCommit} (no inline `content`).
 *
 * @param {string[]} deletions
 * @param {ShaUploadEntry[]} uploads
 * @returns {Array<Array<{path: string, mode: string, type: string, sha: string|null}>>}
 */
export function chunkAtomicCommitShaTreeBatches(deletions, uploads) {
  /** @type {Array<Array<{path: string, mode: string, type: string, sha: string|null}>>} */
  const batches = [];
  ...
  const push = (item) => {
    if (batch.length >= ATOMIC_COMMIT_TREE_BATCH_MAX_ENTRIES) {
      flush();
    }
    batch.push(item);
  };
  ...
  for (const { path, sha } of uploads) {
    push({ path, mode: '100644', type: 'blob', sha });
  }
  flush();
  return batches;
}
